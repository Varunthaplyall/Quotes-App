...
async function fetchQuotes() {
    try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/quotes`)
...
async function submithandle() {
    try {
        const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/quotes`, {
            text
        },{
            headers : {
  ...
  return (
    <div className='pt-20 font-merriweather'>
        <div className='mb-14 border bg-white rounded-lg shadow-lg'>
            ...
            <button type='submit' className='mt-4 w-full bg-green-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-600 transition-all'>                Post
            </button>
            </form>
